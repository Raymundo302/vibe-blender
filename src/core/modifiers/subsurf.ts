...
function subdivideOnce(mesh: EditableMesh): EditableMesh {
  ...
  for (const e of edges.values()) {
    ...
    if (e.faces.length === 2) {
      ...
      edgePoint.set(e.key, a.add(b).add(fp).scale(0.25));
    } else {
      // Boundary (or non-manifold) edge → endpoint midpoint.
  ...
  for (const v of mesh.verts.values()) {
    ...
    if (boundaryEdges.length === 0) {
      ...
      newVertPos.set(v.id, F.add(R.scale(2)).add(P.scale(n - 3)).scale(1 / n));
      ...
      newVertPos.set(v.id, m1.add(m2).add(P.scale(6)).scale(1 / 8));
    } else {
      // Single or >2 boundary edges (non-manifold corner) → keep P.
      newVertPos.set(v.id, P);
    }
  }
  ...
  // Each n-gon → n quads, winding preserving the original orientation.
  for (const f of mesh.faces.values()) {
    const vs = f.verts;
    const n = vs.length;
    for (let i = 0; i < n; i++) {
