...
import { SurfaceEditState } from './SurfaceEdit';
...
export class SceneObject {
  ...
  curve?: CurveData;
  /** NURBS-surface payload (NB-CORE) — set iff kind === 'surface'. The mesh is
   *  the payload's TESSELLATION, regenerated by the surface driver whenever the
   *  payload changes (the text-object pattern). */
  surface?: SurfaceData;
...
export class Scene {
  ...
  /** Non-null while editing one surface object's control net (NB-CORE). */
  surfaceEdit: SurfaceEditState | null = null;
  ...
  /**
   * Add a NURBS-surface object (NB-CORE). Carries a real mesh (the payload's
   * tessellation) that starts EMPTY: the surface driver regenerates it from
   * the payload (pure math, but centralized so undo/animation see one path).
   * Spawns Shade Smooth — tessellated NURBS read as smooth surfaces.
   */
  addSurface(name: string, data: SurfaceData): SceneObject {
    const obj = new SceneObject(this.nextId++, name, new EditableMesh(), 'surface');
    obj.surface = data;
  ...
  /** Enter surface edit mode on a surface object (defaults to the active one). */
  enterSurfaceEdit(id = this.activeId): boolean {
    const obj = id === null ? null : this.get(id);
    if (!obj || obj.kind !== 'surface' || !obj.surface) return false;
    this.selectOnly(obj.id);
    this.surfaceEdit = new SurfaceEditState(obj.id);
    return true;
  }

  exitSurfaceEdit(): void {
    this.surfaceEdit = null;
  }

  /** The surface object being edited, or null outside surface edit mode. */
  get surfaceEditObject(): SceneObject | null {
    return this.surfaceEdit ? this.get(this.surfaceEdit.objectId) ?? null : null;
  }
  ...
  duplicate(src: SceneObject, name: string): SceneObject {
    ...
    // Surface payload (NB-CORE): deep-copy; the cloned mesh above is a valid
    // starting tessellation and the driver keeps it in sync from here.
    if (src.surface) obj.surface = cloneSurfaceData(src.surface);
  ...
  remove(id: number): void {
    ...
    if (this.surfaceEdit?.objectId === id) this.exitSurfaceEdit();
